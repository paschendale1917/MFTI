...
record july_meas[NUM_MEAS]={ 2025,july,20,19,35,45,
                            2025,july,20,19,36,40,
                            2025,july,20,19,37,23,
                            2025,july,20,19,38,36,
                            2025,july,20,19,39,18,
                            2025,july,20,19,40,39
};
...
data jan = {1, 0}, feb = {1, 0}, mar = {1, 0}, apr = {1, 0}, my = {1, 0},
     ...
     nov = {1, 0}, dec = {1, 0};
const char month_name[12][10] = {"January",   "February", "March",    "April",
...
uint64_t encode_date(record *dt) {
  return (uint64_t)dt->year << 32 | (uint32_t)dt->month << 24 |(uint32_t) dt->day<<16|(uint32_t)dt->hour<<8|dt->min;
}
...
void add_record(record *dt, uint8_t num_meas, uint16_t year, uint8_t month,
...
// meas_amount помещаем количество снятых за месяц замеров(при импорте из файла нужно будет отсчитать)
...
// предположительно будет 12 массивов структур,которые будут помещены в структуры
// типа data
// в  массив буду помещать данные по одному из месяцев
// в структуре типа data производится хранение данных за месяц, где meas_amount==количество замеров
...
//получаем адрес структуры типа data, в каждой из которых содержится количество замеров и массив структур
//далее нужно будет написать функцию снюхивания данных по месяцам и раскидыванию их по струтурам типа data,
//  также нужно будет записывать в поле num_meas количество считанных данных по снюханному месяцу
...
void print_title(void){
  printf("%-15s %-14s %-10s %s\n","month","average_temp", "min_temp", "max_temp");
  printf("------------ ---------------- ---------- ----------\n");
}
...
void print_month_info(uint8_t num_month){
  ...
  printf("%s%s %d%s%s %d%s%s %d%s\n",month_name[num_month - 1],"              ",av,"°C","         ", p->measure[0].temp,"°C","       ", p->measure[p->meas_amount-1].temp,"°C");
  printf("\n===============All data for %s===============\n\n",month_name[num_month - 1]);
  printf("%-21s %-14s %-10s\n","date","time", "temp");
  printf("-----------          ------         ------\n");
  sortByDate(p->measure,p->meas_amount);
  for(int32_t i=p->meas_amount-1;i>=0;i--){
   printf("%02u/%02u/%u",p->measure[i].day,p->measure[i].month,p->measure[i].year);
...
void print_year_info(void){
  ...
  av=month_average_temp(p->measure, p->meas_amount);
  sortByTemp(p->measure,p->meas_amount);
  printf("%-19s %-13d %-10d %d\n",month_name[i - 1],av, p->measure[0].temp, p->measure[p->meas_amount-1].temp);
   }
...
void print_yearstat_info(void){
  ...
  for(uint8_t i=1;i<=12;i++){
    ...
    max_temp<p->measure[p->meas_amount-1].temp?max_temp=p->measure[p->meas_amount-1].temp:max_temp;
  }
  printf("\n===============STATISTICS FOR THE YEAR==========\n\n");
  printf("%-15s %-10s %s\n","average_temp", "min_temp", "max_temp");
  printf("-------------  ---------  --------\n");
  printf("%s %d%s%s %d%s%s %d%s\n","    ",av/12,"°C","         ", min_temp,"°C","       ", max_temp,"°C");
}
